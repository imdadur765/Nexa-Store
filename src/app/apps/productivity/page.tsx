...
export default function AppProductivityPage() {
    const { apps, loading } = useApps();

    const productivityApps = apps.filter(app => {
        const isGame = app.isGame || app.category.toLowerCase().includes('game');
        if (isGame) return false;
        return app.tags?.some(t => t.toLowerCase() === 'row:app-productivity') || app.category === 'Productivity';
    });
            ...
            <div style={{ padding: '2rem 1.25rem' }}>
                {productivityApps.length > 0 ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '4rem 0', opacity: 0.5 }}>
                        <p>No productivity apps found.</p>
                    </div>
                )}
            </div>
        </div>
