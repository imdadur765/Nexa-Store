...
export default function AppEntertainmentPage() {
    const { apps, loading } = useApps();

    const entertainmentApps = apps.filter(app => {
        const isGame = app.isGame || app.category.toLowerCase().includes('game');
        if (isGame) return false;
        return app.tags?.some(t => t.toLowerCase() === 'row:app-entertainment') || app.category === 'Entertainment' || app.category === 'Music';
    });

    if (loading) return (
        <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ width: '40px', height: '40px', border: '3px solid rgba(255,255,255,0.1)', borderTopColor: '#ec4899', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
            ...
            <div style={{ padding: '2rem 1.25rem' }}>
                {entertainmentApps.length > 0 ? (
                        ...
                        {entertainmentApps.map(app => (
                            <Link key={app.id} href={`/app/${app.id}`} className="ios-btn-haptic haptic-scale glass-noise" style={{
                                ...
                                <div style={{
                                    ...
                                    width: '80%',
                                    height: '80%',
                                    background: `radial-gradient(circle at center, ${app.accentColor || '#ec4899'} 0%, transparent 60%)`,
                    ...
                    <div style={{ textAlign: 'center', padding: '4rem 0', opacity: 0.5 }}>
                        <p>No entertainment apps found.</p>
                    </div>
                )}
            </div>
        </div>
