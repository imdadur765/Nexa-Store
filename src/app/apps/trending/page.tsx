...
export default function AppTrendingPage() {
    const { apps, loading } = useApps();

    const trendingApps = apps.filter(app => {
        const isGame = app.isGame || app.category.toLowerCase().includes('game');
        if (isGame) return false;

        return app.trending || app.tags?.some(t => t.toLowerCase() === 'row:app-trending');
    });
                        ...
                        {trendingApps.map(app => (
                            <Link key={app.id} href={`/app/${app.id}`} className="ios-btn-haptic haptic-scale glass-noise" style={{
                                ...
                                <div style={{
                                    ...
                                    width: '80%',
                                    height: '80%',
                                    background: `radial-gradient(circle at center, ${app.accentColor || 'rgba(59, 130, 246, 0.4)'} 0%, transparent 60%)`,
