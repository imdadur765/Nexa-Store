...
import { Star, Download, Rocket } from 'lucide-react';
import { PageHeader } from '@/components/PageHeader';

export default function AppNewReleasesPage() {
    const { apps, loading } = useApps();

    const newApps = apps
        .filter(app => !(app.isGame || app.category.toLowerCase().includes('game')))
        .sort((a, b) => {
    ...
    if (loading) return (
        <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ width: '40px', height: '40px', border: '3px solid rgba(255,255,255,0.1)', borderTopColor: '#10b981', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
