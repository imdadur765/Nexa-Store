...
export default function AppStorePortalPage() {
    const { apps, loading } = useApps();

    const appStoreGames = apps.filter(app =>
        (app.isGame || app.category.toLowerCase().includes('game')) &&
        (app.storeSource?.includes('appstore') || app.tags?.some(t => t.toLowerCase() === 'row:game-originals'))
    );
    ...
    return (
        <div className="no-scrollbar" style={{ maxWidth: '1200px', margin: '0 auto', padding: '0.75rem', paddingBottom: '8rem', background: 'var(--bg-primary)' }}>
            <PageHeader
                ...
                icon={<Image src="/platforms/appstore_logo.png" width={20} height={20} alt="App Store" style={{ objectFit: 'contain' }} />}
                accentColor="#ffffff"
                backText="Games"
            />
            ...
            <div style={{ padding: '2rem 1.25rem' }}>
                <div style={{ marginBottom: '2.5rem' }}>
                    <h1 style={{ fontSize: '2.5rem', fontWeight: '900', letterSpacing: '-1.5px', marginBottom: '0.6rem', color: 'white' }}>App Store Originals</h1>
                    <p style={{ fontSize: '1rem', color: 'rgba(255,255,255,0.4)', fontWeight: '600', letterSpacing: '-0.2px' }}>Exclusive on iOS.</p>
                </div>
            ...
            <div style={{
                ...
                width: '100vw',
                height: '100vh',
                background: 'radial-gradient(circle at 110% 110%, rgba(255, 255, 255, 0.05), transparent 50%)',
                zIndex: -1,
                pointerEvents: 'none'
            }} />
        </div>
